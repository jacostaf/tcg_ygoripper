...
def fetch_all_cards_from_sets():
    ...
    try:
        ...
        logger.info(
            f"Completed processing all sets. Found {processing_stats['total_cards_found']} total cards"
        )
...
def upload_card_variants_to_mongodb():
    ...
    try:
        ...
        if all_variants:
            ...
            for i in range(0, len(all_variants), batch_size):
                ...
                try:
                    ...
                    logger.info(
                        f"Inserted batch {i//batch_size + 1}: {len(insert_result.inserted_ids)} variants"
                    )
                except Exception as e:
            ...
            try:
                ...
                variants_collection.create_index(
                    [("card_id", 1), ("set_code", 1), ("set_rarity", 1)]
                )
        ...
        logger.info(
            f"Completed uploading card variants. Created {processing_stats['unique_variants_created']} unique variants"
        )
