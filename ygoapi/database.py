...
class DatabaseManager:
    ...
    def __init__(self):
        ...
        # Check if database connections are disabled immediately
        # Re-read environment variable each time to support test overrides
    ...
    def _is_database_disabled(self) -> bool:
        """Check if database is disabled, re-reading environment variable for tests."""
        return os.environ.get("DISABLE_DB_CONNECTION") == "1"
    ...
    def get_client(self) -> MongoClient:
        ...
        if self._is_database_disabled():
            logger.info(
                "Database connections disabled by DISABLE_DB_CONNECTION environment variable"
            )
            return None
