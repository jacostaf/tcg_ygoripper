...
from .database import get_card_variants_collection, get_price_cache_collection
from .memory_manager import get_memory_manager, monitor_memory
...
class PriceScrapingService:
    ...
    def __init__(self):
        ...
        self.memory_manager.register_cleanup_callback(
            "price_scraper_cleanup", self.cleanup_playwright
        )
    ...
    def _initialize_collections(self):
        ...
        try:
            ...
            if self.cache_collection is None or self.variants_collection is None:
                logger.info(
                    "Database connections disabled, price scraping will run in offline mode"
                )
                return
            ...
            desired_indexes = [
                ...
                {
                    "fields": [("card_number", 1), ("card_rarity", 1)],
                    "name": "card_number_rarity_idx",
                },
            ]
                ...
                if fields not in existing_indexes:
                    try:
                        self.cache_collection.create_index(
                            idx_spec["fields"], name=idx_spec["name"], background=True
                        )
    ...
    def find_cached_price_data(
        ...
        try:
            ...
            normalized_art_variant = (
                self._normalize_art_variant(art_variant) if art_variant else None
            )

            logger.debug(
                f"Searching cache - Number: {card_number}, Rarity: {normalized_rarity}, "
            ...
            query = {
                ...
                "card_rarity": {
                    "$regex": f"^{re.escape(normalized_rarity)}$",
                    "$options": "i",
                },
            }
            ...
            if normalized_art_variant:
                ...
                query["$or"] = [
                    ...
                    {
                        "art_variant": {
                            "$in": self._get_art_variant_alternatives(normalized_art_variant)
                        }
                    },
                ]
            ...
            documents.sort(
                key=lambda x: x.get("last_price_updt", datetime.min.replace(tzinfo=timezone.utc)),
                reverse=True,
            )
            ...
            if is_cache_fresh(last_updated, PRICE_CACHE_EXPIRY_DAYS):
                logger.info(
                    f"Found fresh cached price data for {card_number} (updated: {last_updated})"
                )
                return document
            else:
                logger.info(
                    f"Found stale cached price data for {card_number} (updated: {last_updated})"
                )
                return None
        ...
        try:
            ...
            for available_rarity in available_rarities:
                ...
                if self._are_rarities_equivalent(normalized_requested, normalized_available):
                    logger.info(
                        f"Rarity '{card_rarity}' validated for card {card_number} (equivalent to '{available_rarity}')"
                    )
                    return True

            logger.warning(
                f"No matching rarity found for card {card_number} with rarity {card_rarity}"
            )
    ...
    @monitor_memory
    def save_price_data(
        self, price_data: Dict[str, Any], requested_art_variant: Optional[str] = None
    ) -> bool:
        """
        ...
        try:
            ...
            result = self.cache_collection.replace_one(query, document, upsert=True)
            ...
            if result.upserted_id or result.modified_count > 0:
                ...
                logger.warning(
                    f"No changes made when saving price data for {document['card_number']}"
                )
                return False
    ...
    def lookup_card_name_from_ygo_api(self, card_number: str) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if card_name:
                    logger.info(
                        f"Found card name '{card_name}' for card number {card_number} via YGO cardsetsinfo API"
                    )
            ...
            async with async_playwright() as p:
                ...
                if art_variant:
                    ...
                    if art_search_terms:
                        ...
                        logger.info(
                            f"Searching with art variant: '{search_card_name}' (original: '{card_name}', art: '{art_variant}')"
                        )
    ...
    def scrape_card_price(
        ...
        """
        try:
            # Validate card rarity first
            if not self.validate_card_rarity(card_number, card_rarity):
                logger.warning(f"Invalid rarity '{card_rarity}' for card {card_number}")
                return {
                    "success": False,
                    "card_number": card_number,
                    "card_name": card_name,
                    ...
                    "cached": False,
                    "error": f"Invalid rarity '{card_rarity}' for card {card_number}",
                }

            # Check cache first unless force refresh
            if not force_refresh:
                logger.info(
                    f"Checking cache for card: {card_number}, name: {card_name}, rarity: {card_rarity}, art: {art_variant}"
                )
            ...
            try:
                ...
                price_data = asyncio.run(
                    self.scrape_price_from_tcgplayer_basic(
                ...
                if price_data and not price_data.get("error"):
                    ...
                    logger.warning(
                        f"Price scraping failed for {card_number}: {price_data.get('error', 'Unknown error')}"
                    )
    ...
    async def select_best_tcgplayer_variant(
        ...
        try:
            ...
            for variant in variants:
                ...
                elif card_number and (
                    card_number.lower() in title_lower or card_number.lower() in url_lower
                ):
                    score += 75
