...
class MemoryManager:
    ...
    def force_cleanup(self):
        ...
        usage = self.get_current_memory_usage()
        logger.info(
            f"Memory usage after cleanup: {usage['rss_mb']:.1f}MB ({usage['usage_ratio']:.1%})"
        )
    ...
    def check_memory_and_cleanup(self):
        ...
        if self.is_memory_critical():
            usage = self.get_current_memory_usage()
            logger.warning(
                f"Memory usage critical: {usage['rss_mb']:.1f}MB ({usage['usage_ratio']:.1%})"
            )
            self.force_cleanup()
            ...
            if new_usage["usage_ratio"] >= self.critical_threshold:
                logger.error(
                    f"Memory usage still critical after cleanup: {new_usage['rss_mb']:.1f}MB"
                )

        elif self.is_memory_warning():
            usage = self.get_current_memory_usage()
            logger.info(
                f"Memory usage warning: {usage['rss_mb']:.1f}MB ({usage['usage_ratio']:.1%})"
            )
