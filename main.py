...
TCGPLAYER_MIN_VARIANTS_BEFORE_EARLY_TERMINATION = int(
    os.getenv("TCGPLAYER_MIN_VARIANTS_BEFORE_EARLY_TERMINATION", "10")
)
...
class CardPriceModel(BaseModel):
    ...
    card_art_variant: Optional[str] = Field(
        None, description="Art version (e.g., '7', '1st', etc.)"
    )
    booster_set_name: Optional[str] = Field(None, description="Booster set name where card is from")
    set_code: Optional[str] = Field(
        None, description="4-character set code (e.g., BLTR, SUDA, RA04)"
    )
    card_rarity: str = Field(
        ..., description="Card rarity (e.g., Secret Rare, Ultra Rare) - REQUIRED"
    )
    ...
    last_price_updt: datetime = Field(
        default_factory=lambda: datetime.now(UTC), description="Last price update time"
    )
    ...
    try:
        if sync_price_scraping_client is None:
            ...
            try:
                ...
                sync_price_scraping_collection.create_index(
                    "card_number", name="card_number_idx", background=True
                )
                sync_price_scraping_collection.create_index(
                    "last_price_updt", name="timestamp_idx", background=True
                )
...
def extract_art_version(card_name: str) -> Optional[str]:
    ...
    for pattern in numbered_patterns:
        ...
        if match:
            ...
            logger.debug(
                f"Detected numbered art version: {art_version} using pattern '{pattern}' in: {card_name}"
            )
            return art_version
    ...
    for pattern in named_patterns:
        ...
        if match:
            ...
            logger.debug(
                f"Detected named art version: '{art_version}' using pattern '{pattern}' in: {card_name}"
            )
            return art_version
...
def normalize_rarity_for_matching(rarity: str) -> List[str]:
    ...
    if "quarter century" in normalized:
        if "secret" in normalized:
            ...
            variants.extend(["quarter century ultra rare", "qcur", "25th anniversary ultra rare"])

    # Handle Platinum Secret Rare
    if "platinum" in normalized and "secret" in normalized:
        variants.extend(["platinum secret rare", "psr", "plat secret rare"])

    # Handle Prismatic variants
    if "prismatic" in normalized:
        if "secret" in normalized:
            variants.extend(["prismatic secret rare", "prismatic secret"])
        elif "collector" in normalized:
            variants.extend(["prismatic collector rare", "prismatic collector's rare"])
        elif "ultimate" in normalized:
            variants.extend(["prismatic ultimate rare"])

    # Handle Starlight Rare
    if "starlight" in normalized:
        variants.extend(["starlight rare", "starlight"])

    # Handle Collector's Rare
    if "collector" in normalized:
        variants.extend(["collector's rare", "collector rare", "collectors rare"])

    # Handle Ghost Rare
    if "ghost" in normalized:
        if "gold" in normalized:
            variants.extend(["ghost gold rare", "ghost/gold rare"])
        else:
            variants.extend(["ghost rare", "ghost"])

    # Handle Ultimate Rare
    if "ultimate" in normalized:
        variants.extend(["ultimate rare", "ultimate"])

    # Handle Parallel variants
    if "parallel" in normalized:
        if "ultra" in normalized:
            variants.extend(["ultra parallel rare", "parallel ultra rare"])
        elif "secret" in normalized:
            variants.extend(["parallel secret rare"])
        else:
            variants.extend(["parallel rare", "parallel"])

    # Handle Gold variants
    if "gold" in normalized:
        if "premium" in normalized:
            variants.extend(["premium gold rare", "premium gold"])
        else:
            variants.extend(["gold rare", "gold"])
    ...
    if "platinum" in normalized:
        if "secret" in normalized:
            ...
            variants.extend(["platinum rare", "platinum"])

    # Handle Duel Terminal Rare
    if "duel terminal" in normalized:
        variants.extend(["duel terminal rare", "dt rare"])

    # Handle Mosaic Rare
    if "mosaic" in normalized:
        variants.extend(["mosaic rare", "mosaic"])

    # Handle Shatterfoil Rare
    if "shatterfoil" in normalized:
        variants.extend(["shatterfoil rare", "shatterfoil"])

    # Handle Starfoil Rare
    if "starfoil" in normalized:
        variants.extend(["starfoil rare", "starfoil"])

    # Handle Hobby League Rare
    if "hobby league" in normalized:
        variants.extend(["hobby league rare", "hl rare"])

    # Handle Millennium Rare
    if "millennium" in normalized:
        variants.extend(["millennium rare", "millennium"])

    # Handle 25th Anniversary variants
    if "25th anniversary" in normalized:
        if "ultra" in normalized:
            variants.extend(["25th anniversary ultra rare", "quarter century ultra rare"])
        elif "secret" in normalized:
            variants.extend(["25th anniversary secret rare", "quarter century secret rare"])

    # Handle 20th Secret Rare
    if "20th" in normalized and "secret" in normalized:
        variants.extend(["20th secret rare"])

    # Handle Extra Secret Rare (OCG)
    if "extra secret" in normalized:
        variants.extend(["extra secret rare"])

    # Handle Red/Blue Secret Rare (OCG)
    if ("red" in normalized or "blue" in normalized) and "secret" in normalized:
        variants.extend(["red secret rare" if "red" in normalized else "blue secret rare"])
...
def find_cached_price_data_sync(
    ...
    try:
        ...
        logger.info(
            f"🔍 CACHE LOOKUP: card_number={card_number}, card_name={card_name}, rarity={card_rarity}, art_variant={art_variant} -> normalized: {normalized_art_variant}"
        )
        ...
        documents = list(
            sync_price_scraping_collection.find(
                base_query, projection=projection, sort=[("last_price_updt", -1)]
            ).limit(5)
        )
        ...
        for i, doc in enumerate(documents):
            logger.info(
                f"  📄 Document {i+1}: card_art_variant='{doc.get('card_art_variant', 'N/A')}', card_rarity='{doc.get('card_rarity', 'N/A')}'"
            )
...
def _check_freshness_and_return(document) -> tuple[bool, Optional[Dict]]:
    ...
    try:
        ...
        has_price_data = bool(
            document.get("tcg_price") is not None
            or document.get("tcg_market_price") is not None
            or document.get("pc_ungraded_price") is not None
        )
        ...
        if is_fresh and has_price_data:
            ...
            logger.info(
                f"  ⚠️  Found FRESH cached data but NO PRICING DATA (updated: {last_update}) - treating as stale"
            )
            is_fresh = False  # Treat as stale if no pricing data
        elif not is_fresh and has_price_data:
            logger.info(
                f"  ⚠️  Found STALE cached data with pricing (updated: {last_update}, expired: {expiry_date})"
            )
        else:
            logger.info(
                f"  ❌ Found STALE cached data with NO PRICING DATA (updated: {last_update}, expired: {expiry_date})"
            )
...
def validate_card_rarity_sync(card_number: str, card_rarity: str) -> bool:
    ...
    try:
        ...
        for available_rarity in available_rarities:
            ...
            if (
                normalized_requested in normalized_available
                or normalized_available in normalized_requested
            ):
                logger.info(f"Rarity '{card_rarity}' partially validated for card {card_number}")
                return True

        logger.warning(
            f"Rarity '{card_rarity}' not found for card {card_number}. Available rarities: {list(available_rarities)}"
        )
        return False
...
def save_price_data_sync(price_data: Dict, requested_art_variant: Optional[str] = None) -> bool:
    ...
    try:
        ...
        if requested_art_variant is not None:
            ...
            logger.info(
                f"🔧 Using requested art variant '{requested_art_variant}' for cache consistency (detected art variant was: '{price_data.get('card_art_variant')}')"
            )
        ...
        if normalized_art_variant:
            ...
            logger.info(
                f"🔧 Normalized art variant: '{original_art_variant}' -> '{normalized_art_variant}'"
            )
        else:
        ...
        if price_data.get("card_number") and price_data["card_number"] != "Unknown":
            ...
            deletion_query["card_name"] = {
                "$regex": re.escape(price_data["card_name"]),
                "$options": "i",
            }
        ...
        logger.info(
            f"🗑️ TARGETED CACHE DELETION: Deleting records matching card + rarity + art variant to preserve other variants"
        )
        ...
        if docs_to_delete:
            ...
            for doc in docs_to_delete:
                logger.info(
                    f"    🗑️ Will delete: card_number={doc.get('card_number')}, art_variant={doc.get('card_art_variant')}, rarity={doc.get('card_rarity')}, last_update={doc.get('last_price_updt')}"
                )
            ...
            if remaining_count > 0:
                logger.error(
                    f"  ❌ DELETION FAILED: Still found {remaining_count} records after deletion!"
                )
                remaining_docs = list(
                    sync_price_scraping_collection.find(
                        deletion_query,
                ...
                for doc in remaining_docs:
                    logger.error(
                        f"    🔍 Remaining: card_number={doc.get('card_number')}, art_variant={doc.get('card_art_variant')}, rarity={doc.get('card_rarity')}, last_update={doc.get('last_price_updt')}"
                    )
            else:
        ...
        if result.inserted_id:
            ...
            if cleaned_data.get("card_number") and cleaned_data["card_number"] != "Unknown":
                ...
                verification_query["card_name"] = {
                    "$regex": re.escape(cleaned_data["card_name"]),
                    "$options": "i",
                }
            ...
            if cleaned_data.get("card_art_variant"):
                ...
                logger.info(
                    f"  🎨 Verification will use art variant filter: '{cleaned_data['card_art_variant']}'"
                )
            else:
                logger.info(
                    f"  🎨 No art variant in cleaned data - verification will not filter by art variant"
                )

            logger.info(
                f"🔍 VERIFICATION: Checking if new record was saved with query: {verification_query}"
            )
            ...
            if verification_docs:
                logger.info(
                    f"  ✅ Verification successful: Found {len(verification_docs)} matching record(s)"
                )
                for i, doc in enumerate(verification_docs):
                    logger.info(
                        f"    📄 Record {i+1}: ID={doc.get('_id')}, card_number={doc.get('card_number')}, art_variant='{doc.get('card_art_variant')}', rarity='{doc.get('card_rarity')}', last_update={doc.get('last_price_updt')}"
                    )
            else:
                logger.error(
                    f"  ❌ VERIFICATION FAILED: Could not find the record we just inserted!"
                )
                ...
                if debug_docs:
                    logger.error(
                        f"  🔍 Found {len(debug_docs)} records for this card (without rarity/art filters):"
                    )
            ...
            if card_name:
                ...
                if set_code_prefix:
                    ...
                    variants_query = {
                        "card_name": card_name,
                        "set_code": {
                            "$regex": f"^{re.escape(set_code_prefix)}-",
                            "$options": "i",
                        },
                    }
                    logger.info(
                        f"Looking for variants in set {set_code_prefix} for card {card_name}"
                    )
                else:
                ...
                logger.info(
                    f"Found card info for {card_number}: {card_name}, rarities from set {set_code_prefix or 'all sets'}: {list(set(available_rarities))}"
                )
                return card_info
...
def verify_card_match(
    ...
    if expected_name and tcg_card_name:
        ...
        expected_words = [
            word for word in expected_core.split() if len(word) > 2
        ]  # Skip small words
        ...
        if len(common_words) == len(expected_word_set) and len(expected_word_set) > 0:
            ...
            if not key_conflicts:  # No conflicting descriptive words
                ...
                logger.debug(
                    f"✓ Perfect name match: expected '{expected_name}' matches TCG '{tcg_card_name}'"
                )
            else:
                logger.debug(
                    f"✗ Conflicting words detected: {key_conflicts} in '{tcg_card_name}' vs expected '{expected_name}'"
                )
        else:
            missing_words = expected_word_set - common_words
            logger.debug(
                f"✗ Insufficient name match: missing words {missing_words} from expected '{expected_name}' in TCG '{tcg_card_name}'"
            )
...
def lookup_card_name_from_ygo_api(card_number: str) -> Optional[str]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if card_name:
                logger.info(
                    f"Found card name '{card_name}' for card number {card_number} via YGO API"
                )
                return card_name.strip()
...
def extract_set_code(card_number: str) -> Optional[str]:
    ...
    if "-" in card_number:
        ...
        if len(potential_set_code) >= 2 and potential_set_code.isalpha():
            logger.debug(
                f"Extracted set code (fallback): {potential_set_code} from card number: {card_number}"
            )
...
def extract_booster_set_name(source_url: str) -> Optional[str]:
    ...
    try:
        ...
        set_match = re.search(
            r"/yugioh-([^/]+?)(?:-[^-/]*?-(?:secret|ultra|super|rare|common))",
            source_url,
            re.IGNORECASE,
        )
        if set_match:
        ...
        for part in path_parts:
            if "yugioh-" in part.lower() and len(part) > 10:
                ...
                set_candidate = re.sub(
                    r"\b(Secret|Ultra|Super|Rare|Common)\b.*$",
                    "",
                    set_candidate,
                    flags=re.IGNORECASE,
                ).strip()
...
async def select_best_tcgplayer_variant(
    ...
    try:
        ...
        logger.info(
            f"Found {len(variants)} variants to check - will process ALL variants (no early termination)"
        )

        # Log if we hit the processing limit
        total_available = await page.evaluate(
            "() => document.querySelectorAll('a[href*=\"/product/\"]').length"
        )
        if total_available > max_variants_to_process:
            logger.warning(
                f"Large result set detected: {total_available} total variants found, processing first {max_variants_to_process} (limit)"
            )
            logger.info(
                f"Processing limit applied to prevent excessive processing time. All {max_variants_to_process} variants will still be evaluated."
            )
        ...
        for i, variant in enumerate(variants):
            ...
            if card_name:
                ...
                if variant_card_name and expected_name == variant_card_name:
                    ...
                    if name_match_count == len(name_words):
                        ...
                        logger.info(
                            f"⚠ Partial card name match ({name_match_count}/{len(name_words)})"
                        )
                    else:
                        score -= 100
            ...
            if expected_card_info:
                ...
                if verification_result["overall_match"]:
                    ...
                    logger.info(
                        f"✓ Cache verification passed for {variant['title'][:50]}... (confidence: {verification_result['confidence_score']})"
                    )
                else:
                    logger.info(
                        f"⚠ Cache verification failed for {variant['title'][:50]}... (confidence: {verification_result['confidence_score']})"
                    )
            ...
            if card_rarity:
                ...
                if variant_rarity:
                    ...
                    if variant_rarity_normalized == target_rarity_normalized:
                        ...
                        rarity_found = True
                        logger.info(
                            f"✓ PERFECT rarity match: '{card_rarity}' == '{variant.get('rarity', '')}'"
                        )
                    else:
                        ...
                        target_unique_words = target_words - {
                            "rare",
                            "card",
                        ...
                        variant_distinctive = variant_unique_words.intersection(
                            distinctive_conflicts
                        )
                        ...
                        if (
                            target_distinctive
                            and variant_distinctive
                            and not target_distinctive.intersection(variant_distinctive)
                        ):
                            rarity_score = -300  # Heavy penalty for conflicting rarities
                            logger.warning(
                                f"✗ CONFLICTING rarity types: '{card_rarity}' vs '{variant.get('rarity', '')}' (distinctive words: {target_distinctive} vs {variant_distinctive})"
                            )
                            ...
                            if overlap_ratio >= 0.6:  # At least 60% overlap
                                ...
                                rarity_found = True
                                logger.info(
                                    f"⚠ Weak partial rarity match: '{card_rarity}' ~= '{variant.get('rarity', '')}' (overlap: {overlap_ratio:.1%})"
                                )
                            else:
                                rarity_score = -150
                                logger.warning(
                                    f"✗ Insufficient rarity overlap: '{card_rarity}' vs '{variant.get('rarity', '')}' (overlap: {overlap_ratio:.1%})"
                                )
                        else:
                            # Different rarity - penalty
                            rarity_score = -150
                            logger.warning(
                                f"✗ Rarity mismatch: '{card_rarity}' != '{variant.get('rarity', '')}'"
                            )
                ...
                if not rarity_found:
                    ...
                    logger.warning(
                        f"✗ No rarity match for '{card_rarity}' (extracted: '{variant.get('rarity', 'N/A')}')"
                    )
            ...
            if target_art_version:
                ...
                if variant_art:
                    ...
                    if target_art_clean == variant_art_clean:
                        ...
                        logger.info(
                            f"✓ EXACT art variant match: '{target_art_version}' == '{variant_art}'"
                        )
                    else:
                        # Art variant mismatch - penalty
                        art_version_score = -50
                        logger.warning(
                            f"✗ Art variant mismatch: '{target_art_version}' != '{variant_art}'"
                        )
                else:
                    ...
                    if target_art in title_lower or target_art in url_lower:
                        ...
                        logger.info(
                            f"⚠ Weak art variant match for '{target_art_version}' found in text"
                        )
                    else:
        ...
        if scored_variants and scored_variants[0][0] > 0:
            ...
            if not card_number_present:
                logger.error(
                    f"CRITICAL ERROR: Selected variant does not contain card number {card_number}"
                )
            ...
            if not card_name_match:
                logger.error(
                    f"CRITICAL ERROR: Selected variant does not match card name '{card_name}'"
                )
                logger.error(f"Selected: {best_variant['title']}")
                return None
            ...
            if expected_card_info:
                logger.info(
                    f"Expected from cache: {expected_card_info['card_name']} | {expected_card_info.get('set_rarity', 'Unknown rarity')}"
                )
                logger.info(
                    f"Available rarities in cache: {expected_card_info['available_rarities']}"
                )
            logger.info(f"=" * 80)
    ...
    try:
        ...
        if len(variants) == 1:
            ...
            if card_rarity:
                ...
                try:
                    await page_context.goto(
                        variant["href"], wait_until="networkidle", timeout=15000
                    )
                    ...
                    rarity_matches = (
                        tcg_rarity
                        and normalize_rarity(tcg_rarity).lower()
                        == normalize_rarity(card_rarity).lower()
                    )
                    art_matches = not target_art_version or (
                        tcg_art
                        and str(tcg_art).strip().lower() == str(target_art_version).strip().lower()
                    )

                    if rarity_matches and art_matches:
                        logger.info(
                            f"Single variant matches criteria - Rarity: {tcg_rarity}, Art: {tcg_art}"
                        )
                        return variant["href"]
                    else:
                        if not rarity_matches:
                            logger.warning(
                                f"Single variant has wrong rarity. Found: {tcg_rarity}, Expected: {card_rarity}"
                            )
                        if not art_matches:
                            logger.warning(
                                f"Single variant has wrong art. Found: {tcg_art}, Expected: {target_art_version}"
                            )
                        return None
                finally:
        ...
        matching_variants = [v for v in variants if card_number.lower() in v["title"].lower()]
        ...
        if target_art_version:
            ...
            if art_matching_variants:
                ...
                logger.info(
                    f"Found {len(matching_variants)} variants matching art variant {target_art_version}"
                )
        ...
        for variant in matching_variants:
            ...
            try:
                ...
                if tcg_rarity:
                    ...
                    logger.info(
                        f"Requested rarity: {card_rarity} (normalized: {normalized_requested})"
                    )
                    ...
                    if rarity_matches and art_matches:
                        ...
                        if not art_matches:
                            logger.info(
                                f"✗ Art variant does not match. Found: {tcg_art}, Expected: {target_art_version}"
                            )
                else:
...
async def extract_rarity_from_tcgplayer_page(
    page,
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            if card_number:
                ...
                if expected_card_info:
                    logger.info(
                        f"Found card info in cache: {expected_card_info['card_name']} with rarities: {expected_card_info['available_rarities']}"
                    )
            ...
            # If no card name, try looking it up from cache/API
                ...
                if looked_up_name:
                    ...
                    logger.error(
                        f"Cannot search TCGPlayer without card name. Card number {card_number} not found in cache or YGO API"
                    )
                    await browser.close()
                    return None
            else:
            ...
            if card_number:
                ...
                if set_code:
                    ...
                    if tcgplayer_set_name:
                        logger.info(
                            f"Extracted set code '{set_code}' from card number '{card_number}', mapped to TCGPlayer set: '{tcgplayer_set_name}'"
                        )
            ...
            if card_rarity:
                ...
                if tcgplayer_rarity_filter:
                    logger.info(
                        f"Mapped rarity '{card_rarity}' to TCGPlayer filter: '{tcgplayer_rarity_filter}'"
                    )
            ...
            for search_query, search_type in search_attempts:
                for set_filter, rarity_filter, filter_description in filter_attempts:
                    ...
                    logger.info(
                        f"Searching TCGPlayer for: '{search_query}' with {filter_description}"
                    )
                    if set_filter:
                    ...
                    logger.info(
                        f"Search with {filter_description} returned {results_count} results"
                    )
                    ...
                    if results_count > 0:
                        ...
                        logger.warning(
                            f"No results found for {search_type} with {filter_description}"
                        )
            ...
            if not successful_search:
                logger.error(
                    f"No search query returned results for card {card_number or card_name}"
                )
                await browser.close()
                return None
            ...
            is_product_page = await page.evaluate(
                "() => document.querySelector('.product-details, .product-title, h1[data-testid=\"product-name\"]') !== null"
            )
            ...
            if not is_product_page:
                ...
                logger.info(
                    f"Processing up to {variant_limit} variants from {final_results_count} total results"
                )
                ...
                best_variant_url = await select_best_tcgplayer_variant(
                    ...
                    art_variant,
                    expected_card_info,
                    max_variants_to_process=variant_limit,
                )
            ...
            if art_variant and len(art_variant.strip()) > 0:
                ...
                final_art_variant = (
                    detected_art
                    if detected_art
                    else (actual_art_variant if actual_art_variant else art_variant)
                )
                logger.info(
                    f"Art variant processing: requested='{art_variant}', detected='{detected_art}', extracted='{actual_art_variant}', final='{final_art_variant}'"
                )
            else:
...
def scrape_card_price():
    ...
    try:
        ...
        if card_number:
            ...
            logger.info(
                f"Skipping rarity validation - no card_number provided (using card_name: {card_name})"
            )
...
async def extract_rarity_from_tcgplayer(page) -> tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        try:
            ...
            return (
                rarity.strip() if rarity else None,
                art_variant.strip() if art_variant else None,
            )
...
def get_all_card_sets():
    ...
    try:
        ...
        return jsonify({"success": False, "error": "Failed to connect to YGO API"}), 503
...
def search_card_sets(set_name):
    ...
    try:
        ...
        return jsonify({"success": False, "error": "Failed to search card sets"}), 500
...
def get_card_sets_from_cache():
    ...
    try:
        ...
        return (
            jsonify({"success": False, "error": "Failed to retrieve card sets from cache"}),
            500,
        )
...
def get_card_sets_count():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return (
                jsonify({"success": False, "error": "Failed to fetch data from YGO API"}),
                500,
            )
        ...
        return (
            jsonify({"success": False, "error": "Failed to get card sets count"}),
            500,
        )
...
def filter_cards_by_set(cards_list: List[Dict], target_set_name: str) -> List[Dict]:
    ...
    for card in cards_list:
        ...
        if "card_sets" in card and isinstance(card["card_sets"], list):
            ...
            if filtered_sets:
                ...
                if "card_images" in filtered_card and len(filtered_sets) < len(
                    filtered_card["card_images"]
                ):
                    # Keep only as many images as we have set variants
                    filtered_card["card_images"] = filtered_card["card_images"][
                        : len(filtered_sets)
                    ]
    ...
    logger.info(
        f"Filtered {len(cards_list)} cards down to {len(filtered_cards)} cards with variants in '{target_set_name}'"
    )
...
def get_cards_from_specific_set(set_name):
    ...
    try:
        ...
        return (
            jsonify({"success": False, "set_name": set_name, "error": "Request timed out"}),
            504,
        )
...
def proxy_card_image():
    ...
    try:
        ...
        if not image_url:
            return (
                jsonify({"success": False, "error": "image URL parameter is required"}),
                400,
            )
        ...
        return jsonify({"success": False, "error": "Image request timeout"}), 504
    except Exception as e:
...
def fetch_all_cards_from_sets():
    ...
    try:
                ...
                if response.status_code == 200:
                    ...
                    if include_set_codes and set_code:
                        set_data["set_code_info"] = {
                            "set_code": set_code,
                            "set_code_prefix": set_code.split("-")[0]
                            if "-" in set_code
                            else set_code,
                        }
                    ...
                    logger.info(
                        f"Successfully fetched {len(cards_list)} cards, filtered to {len(filtered_cards)} from {set_name}"
                    )
                    ...
                    processing_stats["processing_errors"].append(
                        {"set_name": set_name, "error": error_msg}
                    )
        ...
        logger.info(
            f"Completed processing all sets. Found {processing_stats['total_cards_found']} total cards, filtered to {processing_stats['total_filtered_cards']}"
        )
